...
> 1. 배경 이미지 재구성
  2. NPC 스프라이트 배치
  3. 스크립트 작성
   - NPC위로 마우스를 올리면 테두리 색이 생성되는 작업
   - 클릭 시 소형 메뉴판이 출력되고 버튼을 클릭해 원하는 작업을 할 수 있다.
   - 소형 메뉴판이 열려있는 상태에서, 소형 메뉴판 버튼을 클릭하지 않고 다른 곳을 클릭하면 메뉴판이 닫히는 기능을 넣자
   - 메뉴판은 대화하기, 특정 UI판 열기로 구성
