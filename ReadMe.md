...
+ 캔디바 타일이 혼자 파괴되는 버그 
+ 스킬 쿨타임 중 스킬 버튼을 누르면 마나가 소모되는 버그
...
0. 몬스터 AI 정확도 및 빈도 설정
1. 3번스킬 AI쪽 관련 추가
2. 캐릭터의 공격, 몬스터의 공격 설계 및 구현

	1) 데바스타르 2페이즈 변신
		1) 체력이 0이 되었을 경우, 데바스타르가 쓰러지며, 잠시 대기시간을 가짐
		2) 데바스타르가 악마로 변신하며 2페이즈가 시작됨.
		3) 변신이 될 때, 체력과 마나가 풀로 채워짐.
		4) 위의 모든 항목이 완료되면, 다시 서로 움직일 수 있게 된다.
