...
여태 설계했던 것을 토대로 앞으로 할 개발 목표를 진행하기에 문제점이 발생했습니다.\
> 타일안의 스프라이트를 교체하는 방식의 구현이 아닌, 타일 오브젝트 끼리의 교체로 설계했어야 했다.
  이에 따라, 코드의 전체 구조를 바꿔야한다.
이에 따라 이 repository는 더 이상 개발되지 않습니다.\

구상과 설계를 점검하여 Repository를 새로 생성합니다. 이후 개발은 아래 주소를 참조하세요.
