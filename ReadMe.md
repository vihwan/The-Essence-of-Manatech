...
> 타일과 백그라운드 타일을 생성할 때, 현재 스크린의 크기에 알맞는 사이즈로 재조정하여 생성해야한다.

	1920(reference size) : 80 = Screen.width : x  
	와 같이 설정하면 되지 않을까??
...
+ (확인중) **가끔씩 매칭이 되어도 파괴가 이루어지지 않는 버그**
...
1. 프로토타입 피드백 수정
	...
	* **(진행중) 해상도에 따라 타일 크기가 올바르게 바뀌지 않는 버그
	> 이전에 확인했었던 버그. 타일 생성 시, 화면 크기에 맞게 타일의 사이즈를 재조정하지 않았기 때문에 생기는 버그라고 생각
	 ...
	 타일 이미지의 크기도 바꿔야하는데, SetParent에서 뒤에 WorldPositionStays 패러미터를 이용하여 false로 지정해주면 알아서 바뀜.. 미친;;
	 1. 일일이알아서 바꾸는 방법
	 2. Scale?
	 3. transform.SetParent WorldPositionStays**
